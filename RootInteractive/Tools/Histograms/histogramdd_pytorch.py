...
# This is done in order for the function to accept a "range" argument
_range = range
...
def histogramdd(sample, bins=None, range=None, weights=None, remove_overflow=True):
    ...
    :param sample:            Tensor
        D by N Tensor, i-th row being the list of positions in the i-th coordinate, j-th column being the
        coordinates of the j-th item
    ...
    :param range:           sequence, optional
        Each item in the sequence is either a (min,max) Tuple, or None, in which case the edges
        are calculated as the minimum and maximum of input data
    ...
    if custom_edges:
        ...
        # If the range consists of only one point, avoid dividing by 0
        singular_range = torch.eq(range[0], range[1])
        ...
        # Underflow bin
        k = torch.max(k, torch.zeros([], device=device, dtype=torch.long))
        k = torch.min(k, (bins+1).reshape(-1, 1))
    ...
    binindex = torch.sum(k, 0)
    hist = torch.bincount(binindex, minlength=(multiindex[0]*(bins[0]+2)), weights=weights)
