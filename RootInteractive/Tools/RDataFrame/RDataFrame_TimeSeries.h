...
auto getStat1(const ROOT::RVec<DataVal>& vecRef, const ROOT::RVec<DataTime>& timeRef, const ROOT::RVec<DataTime>& time0, std::vector<std::string> statVector, float deltaMax)
{
  ...
  if(hasMedian){
    ...
    auto statMedian = statMap["median"];
    for (int i = 0; i < vecSize; i++) {
      DataTime timeI = time0[i];
      DataTime timeMin = timeI-deltaMax;
      DataTime timeMax = timeI+deltaMax;
      ...
      while(imbalance < -1){
        while (queueLow.top() < indexMin){
          queueLow.pop();
        }
        int y = queueLow.top();
        queueHigh.push(y);
        queueLow.pop();
        closedSet[y-indexMin] = -1; 
        imbalance ++;
      }
      while(imbalance > 1){
        while (queueHigh.top() < indexMin){
          queueHigh.pop();
        }
        int y = queueHigh.top();
        queueLow.push(y);
      ...
      if(imbalance == 1){
        statMedian[i] = vecRefVal[queueHigh.top()];
      } else if(imbalance == 0){
        statMedian[i] = (vecRefVal[queueHigh.top()] + vecRefVal[queueLow.top()]) / DataVal(2);
      } else if(imbalance == -1){
        statMedian[i] = vecRefVal[queueLow.top()]
      } 
    }
  }
